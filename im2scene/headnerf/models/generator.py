...
class Generator(nn.Module):
    ...
    def forward(self, batch_size=32, latent_codes=None, camera_matrices=None,
        ...
        rgb_v = self.volume_render_image(
            latent_codes, camera_matrices,return_alpha_map=False,
            mode=mode, it=it)

        rgb = self.neural_renderer(rgb_v, it=it)
        return rgb
    ...
    def get_latent_codes(self, batch_size=32, tmp=1.):
        ...
        z_id = torch.randn((batch_size, z_id_dim), device=self.device)
        z_exp = torch.randn((batch_size, z_exp_dim), device=self.device)

        # Retrieve identity and expression latent code from 3DMM
        z_identity = ...
        z_expression = ...

        # TODO: add albedo and illumination latent codes
        #z_albedo = ...
        #z_illumination = ...
    ...
    def get_evaluation_points(self, pixels_world, camera_world, di,
        ...
        pixels_world_i = pixels_world
        camera_world_i = camera_world

        # Only running code for 1 box - the head in the center
    ...
    def volume_render_image(self, latent_codes, camera_matrices,
                            mode='training', return_alpha_map=False,
                            it=0):
        ...
        z_id, z_exp, z_exp_joints, z_albedo, z_detail = latent_codes
        ...
        n_boxes = 1 #latent_codes[0].shape[1]
        feat, sigma = [], []
        n_iter = 1 #n_boxes if not_render_background else n_boxes + 1
        ...
        for i in range(n_iter):
            if i < n_boxes:  # Object
                p_i, r_i = self.get_evaluation_points(
                    pixels_world, camera_world, di, i)

                feat_i, sigma_i = self.decoder(p_i, r_i, z_id, z_exp, z_exp_joints, z_albedo, z_detail)
                # feat_i, sigma_i = self.decoder(p_i, r_i, z_id, z_exp)
                ...
                print('No rendering of background in HeadNerf!')
        ...
        if self.sample_object_existance:
            print('No sampling of object existence for headnerf!')
