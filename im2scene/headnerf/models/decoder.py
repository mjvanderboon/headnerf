...
class Decoder(nn.Module):
    ...
    def __init__(self, hidden_size=128, n_blocks=8, n_blocks_view=1,
                 ...
                 z_id_dim=100, z_exp_dim=50, rgb_out_dim=128, final_sigmoid_activation=False,
        ...
        self.fc_z = nn.Linear(z_id_dim, hidden_size)
        ...
        if n_skips > 0:
            self.fc_z_skips = nn.ModuleList(
                [nn.Linear(z_id_dim, hidden_size) for i in range(n_skips)]
            )
        ...
        self.fc_z_view = nn.Linear(z_exp_dim, hidden_size)
    ...
    def forward(self, p_in, ray_d, z_id=None, z_exp=None, **kwargs):
        # TODO: this is the 'implicit' function from HeadNeRF paper. Volume rendering is done in generator.py
        ...
        #     batch_size = p_in.shape[0]
        #     if z_id is None:
        #         z_id = torch.randn(batch_size, self.z_dim).to(p_in.device)
        #     if z_exp is None:
        #         z_exp = torch.randn(batch_size, self.z_dim).to(p_in.device)
        p = self.transform_points(p_in)
        net = self.fc_in(p)
        if z_id is not None:
            net = net + self.fc_z(z_id).unsqueeze(1)
        net = a(net)
